...
**FlyCV** 是一个 **高性能** 的计算机图像处理库。我们目前在 **ARM** 架构下做了很多优化，相比其他图像处理库性能更为出色。同时，我们也正在开展其他CPU和异构平台的性能优化工作，例如x86，RISC-V，GPU，DSP等。除了速度更快之外，FlyCV提供了更加细粒度的编译选项控制，这使得我们在库体积上会 **更加轻量** 。另外我们在编译阶段还提供了 **自定义命名空间** 的编译选项支持，可以方便快速地解决相同依赖库冲突的问题。

接下来，我们通过一个例子简单体验一下FlyCV和OpenCV在性能和效果上的差异。
