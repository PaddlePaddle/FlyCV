...
public:
    ...
    void operator()(const Range& range) const {
        ...
        const unsigned char* src2_ptr = _src2_ptr + range.start() * 3;
        unsigned char* dst_ptr = _dst_ptr + range.start() * 3;
        int nn_neon = range.size() >> 4;
        int nn_remain = range.size() & 15;
...
private:
    const unsigned char *_src1_ptr;
    const unsigned char *_src2_ptr;
...
int add_weighted_neon(
    ...
    AddWeightedParallelTask task(src1_ptr, src2_ptr, dst_ptr, alpha, beta, gamma);
    parallel_run(Range(0, nn), task);
#else
